...
function recordsFor(records, queryObj, logger) {
  ...
  return records.map(item => {
    ...
    return { ts: Number(item.ts), dn: splitted[0], prop: splitted[1], val: item.val };
  });
}
...
function convertBigint2Number(arr) {
  for (let i = 0; i < arr.length; i++) {
    const row = arr[i];

    if (typeof row.ts === 'bigint') row.ts = Number(row.ts);
    if (typeof row.start === 'bigint') row.start = Number(row.start);
    if (typeof row.end === 'bigint') row.end = Number(row.end);
  }
}
