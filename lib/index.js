...
module.exports = async function (channel, opt, logger) {
  ...
  const SEQ_TABLES = {
    ...
    customtable: 'customtable_id_seq',
    formulas: 'formulas_id_seq',
    recids: 'recids_id_seq',
  };
  ...
  async function getDBSize() {
    try {
      ...
      logger.log('getDBSize ERROR: ' + util.inspect(e), 1);
    }
  ...
  async function write(id, payload, table) {
    ...
    const seqName = SEQ_TABLES[tableName];

    let allColumns;
    let values;
    logger.log(seqName)
    if (seqName) {
      // Убираем id из списка колонок, если он там есть — он пойдёт через nextval
      const dataColumns = columns.filter(c => c !== 'id');
      allColumns = ['id', ...dataColumns];

      values = utils.formValues(payload, dataColumns);
      if (!values || !values.length) return;

      // Каждую строку оборачиваем с nextval в начале
      const values1 = values.map(i => `(nextval('${seqName}'), ${i})`).join(', ');
      const sql = `INSERT INTO ${tableName} (${allColumns.join(', ')}) VALUES ${values1}`;
  ...
  async function writeCustom(id, payload, table) {
    ...
    const values = payload.map(i => `(nextval('customtable_id_seq'),'${i.name}', ${i.ts}, '${i.payload}'::JSON)`).join(', ');
  ...
  */
  ...
  async function deletePoints(tableName, archDay, arrId) {
    ...
    // Приводим ID к числам, отбрасываем мусор
    const ids = arrId.map(i => Number(i.id)).filter(Number.isFinite);
    if (!ids.length) return;

    // Массив чисел → JSON-строка для передачи в UNNEST
    const idsJson = JSON.stringify(ids).replace(/'/g, "''");

    try {
      // 1. Создаём временную таблицу с ID
      await client.run(`
            CREATE OR REPLACE TEMP TABLE _del_ids AS 
            SELECT unnest FROM unnest('${idsJson}'::BIGINT[])
        `);

      // 2. Один DELETE с JOIN через IN (DuckDB сам выберет hash join)
      const sql = `
            DELETE FROM ${tableName}
            WHERE ts < ${delTime}
              AND id IN (SELECT unnest FROM _del_ids)
        `;
      logger.log('deletePoints SQL: ' + sql, 1);

      const changes = await client.query(sql);
      logger.log(`Row(s) deleted ${changes}`, 1);

      // 3. Чистим временную таблицу
      await client.run(`DROP TABLE _del_ids`);
  ...
  async function read(id, queryObj) {
    ...
    try {
      ...
      const result = await client.query(utils.normalizeSql(queryStr));
...
function getColumns(tableName) {
  switch (tableName) {
    case 'timeline':
      return ['id', 'dn', 'prop', 'start', '"end"', 'state'];
    case 'customtable':
      return ['id', 'name', 'ts', 'payload'];
    case 'formulas':
